...
def resize_video(self, input_file: str, output_file: str, height: int, cuda=False):
    if cuda:
        ...
        ffmpeg.input(input_file).filter('scale', 'trunc(oh * (iw/ih)/2) * 2', height).output(
...
def resize_video_with_progress(self, input_file: str, output_file: str, height: int, media_length: int, cuda=False):
    if cuda:
        ff_process = ffmpeg.input(input_file, hwaccel='cuda').output(output_file,
                                                                     vf=f'hwupload_cuda,scale_cuda=w=-1:h={height}',
                                                                     vcodec='h264_nvenc').overwrite_output().run_async(
            pipe_stderr=True, pipe_stdout=True)
    else:
        # CPU encoder needs width divisible by 2, otherwise it will fail
        ff_process = ffmpeg.input(input_file).filter('scale', 'trunc(oh * (iw/ih)/2) * 2', height).output(
            output_file).overwrite_output().run_async(pipe_stderr=True, pipe_stdout=True)

    # Regex to extract time progress (time=00:00:03.00)
    time_re = re.compile(r'time=(\d+:\d+:\d+\.\d+)')

    # FFmpeg doesn't print new lines, it overwrites the same line in stderr. So, we will read continuously from stderr.
    progress = ''
